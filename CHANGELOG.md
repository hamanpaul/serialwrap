...
- 四份 agent 檔（`CLAUDE.md`/`AGENTS.md`/`GEMINI.md`/`.github/copilot-instructions.md`）新增「MCU flash 真機驗證手法（#55 `/dev/ttyMCU`）」段：throwaway daemon + FTDI-only sandbox `SERIALWRAP_BY_ID_DIR` 隔離跑法、DUT console GPIO BSL-invoke（長指令需逐行）、以及三個只有實機才現形的坑（idle 汙染／double-sync 吃 ACK／PTY 無 EOF 卡 FLASHING）。（#55 `/dev/ttyMCU`）」段：throwaway daemon + FTDI-only sandbox `SERIALWRAP_BY_ID_DIR` 隔離跑法、DUT console GPIO BSL-invoke（長指令需逐行）、以及三個只有實機才現形的坑（idle 汙染／double-sync 吃 ACK／PTY 無 EOF 卡 FLASHING）。
