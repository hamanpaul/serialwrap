...
- **flash 偵測 probe 期間取得 bridge 寫入仲裁，消除與 human console 的雙寫入者競態（#83 RACE-2）**：`_on_flash_open` 先 `detect_mcu_line` probe 候選、命中後才 `enter_flashing`；probe 期間 `flash_mode` 仍為 False，候選 bridge 上若有 human raw console 正鍵入，其 `send_bytes` 會與 probe 的 sync bytes 在同一 UART 交錯（兩個邏輯寫入者 → 污染 target 輸入行／干擾 ACK 判讀；calibrated medium）。改為 `_BridgeProbe.probe` 在 `flash_tx`+ACK-wait 視窗暫時開 `flash_mode` gate 取得該 bridge 寫入仲裁：console→device 寫入被 drop，而 probe 的 `flash_tx`（帶 `_allow_during_flash`）與 device→buffer 的 ACK 擷取（`_handle_serial_rx` 不看 flash_mode）皆不受影響；命中與否都於 `finally` 還原（候選必為非 FLASHING）。**至此 #83 RACE-1/STA-4/RACE-2 全數收斂，issue 結案**。新增 probe 持/還原 gate 的回歸測試。
- **事件規則 regex 改用 google-re2 線性引擎，徹底免疫 ReDoS（含多項式殘餘，#91）**：標準 `re` 於 catastrophic backtracking 期間獨佔 GIL 且不可中斷，AST 靜態拒絕只能擋指數類、序列式 `a.*a.*a.*X` 這類**多項式**仍可凍結單執行緒 matcher（#88 後續，#91 追蹤）。`PatternMatcher` 改為：若可匯入 `re2`（可選依賴 `pip install serialwrap[redos]` / `google-re2`）即以其線性引擎（Thompson NFA、無回溯）求值，免疫所有 catastrophic backtracking；未安裝或遇 re2 不支援構造（backreference／lookaround）則自動退回標準 `re` + 既有 schema AST 拒絕 + 輸入封頂防護。flags 以 inline `(?ism)` 帶入、群組擷取語意與 `re` 一致。CI 併裝 `google-re2` 覆蓋 re2 路徑。新增 re2 免疫多項式、flags/群組、backref 退回三組回歸（`importorskip`）。**#91 結案**。
