...
以下保留歷史 bench 結果，不是 #166 候選的實機驗收。#166 增加工具 fallback 及
profile 行長預算後，F7 需使用對應版本重新執行；受影響板須先依量測設定
`max_console_line_chars`，不能用舊的「缺 base64」SKIP 證明新 fallback 不可用，
也不能以本地 shell／pytest 結果宣稱真板已轉綠。
