...
- readiness reprobe（`_prepare_reprobe_locked` 與 worker 寫入前的最終驗證 `_reprobe_target_still_valid_locked`）：不 fire、不累加 attempts、不排 backoff。（#162：兩函式另放行「READY＋`ready_reconfirm_pending` 且 quiet 已結束」的**確認 probe**——沿用既有 RX-idle／backoff／human-active／single-flight 守衛，見下方 #162 段。）
...
**解除（clear）**：RX 匹配該 session 的 `login_regex` / `prompt_regex`（開機完成訊號）即刻解除並恢復探測；否則過期自動解除。例外：若命中的尾行本身就是該 session `bootloader_prompts` 名單中的一員（如 U-Boot 自己的 `=> `），視為仍在 bootloader、非開機完成，本輪不解除——寬鬆撰寫的 `prompt_regex`（如 brcm-template `(?m)[>#]\s*$`）會誤配 bootloader 自身 prompt，若不先排除會在板子正卡 bootloader 的最危險時刻誤解除（`_update_boot_quiet_locked` 於解除前先呼叫 `_matches_any_bootloader_prompt`）。（#162：解除／過期只結束 **TX 靜默**維度；agent 顯式命令 gate 的解除另綁 READY 再確認，見下方 #162 段。）

**不 gate**：human console bytes、interactive lease TX。與 #114「刻意進 bootloader」相容——human/lease 送鍵永遠放行。

**agent 顯式命令的過渡態 gate（#139 雙層 gate；#162 清空判定改綁 READY 再確認）**：`boot_quiet_until` 仍**不**降級 `session.state`，但 agent 顯式命令（`command.submit`／`file.push`／`file.pull`）在「quiet 已 arm 且 session 尚未重新確認 `READY`」的過渡態（自發重開機、state 名義上停 `READY`）被 **`AUTOBOOT_QUIET`**（可重試）拒絕——雙層 gate：(1) submit-time（`service._resolve_session_id`，判「quiet 剩餘 or `ready_reconfirm_pending`」；quiet 進行中 `retry_after_s`＝剩餘秒數、pending-only 固定 `READY_RECONFIRM_RETRY_S=5.0`；拒絕回應附 session dict、不產生 cmd_id）；(2) execute-time（`execute_command`／`file_push`／`file_pull` 的 locked 開頭改判統一入口 `agent_gate_active()`＝`boot_quiet_active() or ready_reconfirm_pending`，堵「submit 通過後 banner 才到、命令已在 arbiter queue」的競態，arbiter 把 error_code 寫回命令記錄、`cmd status` 可觀測）。兩層皆零 UART 副作用、命令可於 READY 再確認後重送。

**#162 清空判定＝READY 再確認（nonce probe），probe 同時消耗 askconsole 啟用 banner**：`arm_boot_quiet()` 同步置 transient 欄位 `ready_reconfirm_pending=True`；quiet 過期或 RX prompt 解除都**不**清它，僅各 READY 確認點呼叫 `confirm_ready()`（＝`clear_boot_quiet()`＋清 pending）解除——確認點＝`_probe_existing_bridge` ok／`_attach_by_id` 兩路徑 READY 落定／`_spawn_reboot_recovery` ok／`_handle_reboot_command` 的 2s prompt-return／`_self_test_impl` READY nonce 成功。根因：prpl/OpenWrt askconsole 停在「`Please press Enter to activate this console`」，既不匹配 `login_regex` 也不匹配 `prompt_regex` → quiet 只能靠 180s 過期清空；舊行為過期即放行，第一個 agent 命令的 `\n` 觸發 askconsole 啟用、命令文字被 askfirst 吞掉、stdout 吃到啟用 banner（marker 遺失、status=done 假成功）。現由 reprobe 引擎在 quiet 結束、RX 靜默（`REPROBE_RX_IDLE_S`）後放行「READY＋pending」的確認 probe（`\n`＋nonce **順帶消耗啟用 banner**），成功即 `confirm_ready()`；失敗則誠實降 `ATTACHED` 走既有 reprobe 流程。quiet 早退分支（`_probe_existing_bridge`）對 `state==READY` 不覆寫 `last_error=PROMPT_UNAVAILABLE`（名義 READY 的 last_error 恆 `null`）。刻意進 bootloader 請走 `interactive-open --allow-attached`（#114，不受 gate）。

**觀測**：session 公開欄位 `boot_quiet_remaining_s`（剩餘秒數；`null`＝未啟用/已解除）、`ready_reconfirm_pending`（#162；`true`＝agent gate 因待 READY 再確認而持續）、`ready_reconfirm_remaining_s`（#162 有界化；pending 距 deadline 的剩餘秒數，非 pending 時 `null`）、`ready_reconfirm_failed`（#162 有界化；`true`＝已落 `READY_UNCONFIRMED` 終態）。

**#162 有界化與失敗終態（回歸修）**：`ready_reconfirm_pending` 必須有界——初版只有一個 clearer（`confirm_ready()`，且要求對 UART probe 成功），只要確認 probe 的任一守衛長期不成立（`reprobe_exhausted` latch 僅於 READY **轉移**點解開，而「已經是 READY」的 session 不會再轉移；RX 洪水下 `_rx_idle_enough` 永不成立；human console 持續活躍；RX 週期性出現字面 `U-Boot` 重臂 quiet），pending 就永久 `true`，`AUTOBOOT_QUIET`（可重試）遂成為**永遠不可能成功的可重試錯誤**。修法：`arm_boot_quiet()` 於 pending 由 `false` 翻 `true` 的**那一刻**起算 `READY_RECONFIRM_MAX_S`（300s＝180s quiet＋120s）deadline 與 `READY_RECONFIRM_MAX_ATTEMPTS`（5）次數上限，**重臂不延長 deadline**；新增 pending 的第二個也是最後一個 clearer `expire_ready_reconfirm(now, *, force=False)`，逾越即置 `ready_reconfirm_failed=True`。終態後四個 gate（`execute_command`／`file_push`／`file_pull`／`service._resolve_session_id`）改回**不可重試**的 `READY_UNCONFIRMED`（不帶 `retry_after_s`、帶 `recommended_action="self_test"`）。`_prepare_reprobe_locked` 的 `reprobe_exhausted` 早退在 READY 再確認分支不再靜默凍結 gate，改為 `force` 落終態。

**#162 autoboot 保護補強（回歸修，實機事故 2026-07-31）**：(1) `_recover_after_failure` 的 CTRL_C/CTRL_D gate 改為**逐 byte 重驗**——兩次寫入間隔 `min(timeout_s, 2.0)`，恰好落在 U-Boot autoboot 3s 倒數窗內，只在迴圈外評估一次會讓 CTRL-D 在 quiet armed 之後仍寫出（WAL 物證：`\x03`@23:39:11.414 → banner arm@23:39:11.948 → `\x04`@23:39:13.424 → `=> `）；(2) `_update_boot_quiet_locked` 的 banner 比對窗改為「舊 tail 尾段＋**整個** chunk」，只有保存才截 `BOOT_BANNER_TAIL_CHARS`——舊式 `(tail+chunk)[-256:]` 會截掉大 chunk 的**開頭**，實機兩個含 `U-Boot` 的 banner chunk（345/354 字元）都因此漏判；(3) `_handle_reboot_command` 的 2s prompt-return **移出** READY 確認點清單、改回只 `clear_boot_quiet()`——prpl/OpenWrt 的 `reboot` 非同步、shell 於 11ms 內重印 prompt 但板子確實在重開，該回顯不是 READY 實證。

**#162 bootloader 卡死的可診斷終態**：readiness probe 失敗且 RX tail 尾行命中 bootloader prompt 時，`_finish_probe_reprobe` 落`last_error=BOOTLOADER_STUCK`（附 `last_error_detail=matched_prompt=<pattern>`）＋`reprobe_exhausted=True`＋`expire_ready_reconfirm(force=True)`，取代舊版「第 10 次靜默 exhausted、state/last_error 不變、不發事件」；`self_test`／`recover_session` 據此回 `classification="BOOTLOADER"` ＋ `recommended_action="recover_interactive"`。比對樣式優先 `profile.bootloader_prompts`，缺配置時退回 `UBOOT_FALLBACK_PROMPTS`（`^=> $`／`^U-Boot> $`／`^CFE> $`），使線上 profile 漂移下偵測仍成立；漂移由 `serialwrap doctor` 的 advisory 檢查 `profile_bootloader_prompts` 指出。READY 再確認 probe 於寫 bytes 前亦先驗 bootloader 證據，命中即不送、直接落終態。
