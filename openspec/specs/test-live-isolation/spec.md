# test-live-isolation Specification

## Purpose
測試套件對 live 資源（state.json／WAL／config.yaml／production daemon）的零接觸保證與其可執行防線：state 路徑注入點、conftest 強制 env 隔離、session-finish live guard、雙 runner 的 state 維度安全（#120）。
## Requirements
...
`tests/conftest.py` SHALL 於 sessionstart 快照、sessionfinish 比對四個 live 維度，任一 FAIL 時 MUST 使 pytest exit code 非零；判定邏輯 SHALL 為純函式（`tests/liveguard.py`）以便單元測試。live 路徑計算 SHALL 採 XDG 公式（`(XDG_STATE_HOME|~/.local/state)/serialwrap/...`、`(XDG_CONFIG_HOME|~/.config)/serialwrap/config.yaml`）並 MUST 忽略 `SERIALWRAP_*` 隔離變數。判定：(1) live state.json 從不存在變存在、或任何 byte 變更 → FAIL；(2) live WAL `raw.wal.ndjson` 消失或 size 縮小 → FAIL（append 視為 live daemon 合法活動）；外層 shell 原有 `SERIALWRAP_WAL_DIR` 路徑任何變更（含同 size 內容改寫）→ FAIL；(3) live config.yaml 任何 byte 變更 → FAIL；(4) live daemon 以多層探測取得快照（system systemd → user systemd → 唯讀 RPC；systemd 未命中但 RPC 可達＝on-demand 情境，仍比對 session 級；systemd 與 RPC 全不可達 → 該維度靜默 SKIP）：pre 有 systemd unit（MainPID 非空）時 MainPID 變更或轉 inactive → FAIL，且以唯讀 RPC 快照比對任一 session 的 `last_tx_at` 前進、`bridge_generation` 變更、`state` 變更或 session 消失 → FAIL。環境變數 `SERIALWRAP_LIVE_GATE=warn` SHALL 將非結構級變更降為警告——降級範圍：Guard 1 非結構性內容變更、Guard 2 size 縮小（rotation 誤報情境）、Guard 4 `last_tx_at`／`bridge_generation`／`state`／session 消失（對真板操作情境）；warn 為明知風險的 opt-in：daemon TX/state 變更僅示警，開發者需自行確認。結構級判定 MUST 不受閥管仍 FAIL——Guard 1 結構性破壞（`released`／`bindings`／`aliases` 任一 key 消失、既有 binding 值被改寫，或內容新出現 `/tmp/sw-`、`test-tpl`、`"test:`、`fake-uart` 污染特徵）、Guard 2 檔案消失、Guard 3 與 shell WAL 維度全部、Guard 4 unit 轉 inactive／MainPID 變更。
