# bench-enrollment-tools Specification

## Purpose
定義 bench 入列所需的 provider-specific 佈署腳本契約，範圍限於 tools/。
管理端憑證與 DNS 由 issue 腳本產出並以 bundle 交付，bench 端不需帳號級憑證；
enroll 腳本須以檢查點驗證成果、失敗非零退出，且不得改動既有 serialwrap 設定。
