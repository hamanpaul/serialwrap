# remote-code-alias Specification

## Purpose
定義 host 端以「一台 bench 一個不變代號」定址遠端 UART 的別名層契約。
別名表只承載 provider-neutral 的 ssh 資訊，代號展開後重用既有 remote -L 路徑；
endpoint 記憶與解析優先序須明確，未建立隧道的代號不得靜默落回本機 daemon。
