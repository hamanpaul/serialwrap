...
- [x] 1.1 寫 `test_com_rank.py`：亂序/並發 attach 一組 fake 裝置 → 斷言 COM 依 sorted `device_key`（含 by-path fallback）；現況應 RED（race/順序）
- [x] 1.2 寫 restart-rank 測試：同組裝置重建 SessionManager → COM↔by-id 不變
- [x] 1.3 寫 `session renumber` 測試：亂序後 renumber → COM snap 回 sorted；斷言 alias/binding/in-flight 參照一致（強制含 active）
...
- [x] 3.1 `SessionManager.renumber_dynamic()`：單一 lock 區間內依 sorted by-id 重排 dynamic COM
- [x] 3.2 原子 remap：`_sessions` key、`_aliases`、`_binding_overrides`、arbiter worker 對應、in-flight `cmd_id`、console/interactive lease、`state.json`
- [x] 3.3 RPC `session.renumber` 分支（`service.py` 平面 dispatcher）
- [x] 3.4 CLI `session renumber` subparser + help（`cli.py`）
- [x] 3.5 跑 1.3 轉綠
...
- [x] 5.2 `README.md`：doctor / daemon status JSON 契約段 + `session renumber` 用法
