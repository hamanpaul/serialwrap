...
- **新增 `session renumber`（RPC + CLI）**：on-demand 把所有 dynamic session 的 COM 依 sorted by-id 重排，無條件強制執行（含 active）。
...
### New Capabilities
- `com-identity-binding`: COM 編號確定性綁定到裝置 by-id（startup sorted rank、rank 作用域、runtime hotplug 沿用、`session renumber` on-demand 重排）。
...
- **程式**：`sw_core/session_manager.py`（COM rank 分配、renumber、rank 作用域）、`sw_core/service.py`（`session.renumber` RPC、`daemon status` 欄位、startup 兩入口）、`sw_core/cli.py`（`session renumber` subparser）、`sw_core/doctor_cmd.py`（`_check_single_daemon`）、新增 module-level /proc 偵測 helper、`sw_core/device_watcher.py`（排序鍵語意確認）。
- **對外契約**：`session renumber` 新命令、`daemon status` 新欄位（`multi_open`/`foreign_holders`）、doctor 新檢查項 → README / `docs/serialwrap-spec.md` 對齊（R-18）。
- **持久化**：`state.json` 在 renumber 後 session_id remap；無新增持久化欄位（COM 確定性來自排序）。
