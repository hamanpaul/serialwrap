...
8. **remediation＝snapshot 通道而非修復**：`enabled: true`（disabled 時 core 不寫 failure_snapshot、全 FAIL 退化 Inconclusive）＋動作鎖死（max_attempts:1 使 on_retry 永不 dispatch；不覆寫 decision hooks＝decision 恆 None，為唯一真正生效的阻擋點——core 對空 `allowed_actions` 為 falsy 短路、不可依賴空白名單）；`hooks.enabled_hooks` 必含 `on_failure`。契約檔單測釘死此組態。
