...
- [ ] 1.1 RED＋GREEN：`CaseResult` 增 `category`/`reason_code`（預設空、向後相容）＋report render 分類欄——`tests/test_realhw_harness.py`
- [ ] 1.2 既有 29 case 逐案標註 category/reason_code（依裁決線：斷言失敗=test、明確外因=environment、執行期 SKIP=environment、未捕捉例外兜底=空+uncaught_exception）
- [ ] 1.3 本機快速驗證：`--only p1-cmd-file`（SKIP 帶 base64_missing）與任一 PASS case 報表分類欄正確

## 2. Phase 1b — preflight 兩級判決（realhw）

- [ ] 2.1 RED＋GREEN：capabilities dict（版本比較、remote 子命令探測、docker 可達）與 requires→執行期 SKIP 映射——`tests/test_realhw_preflight.py`
- [ ] 2.2 RED＋GREEN：benchlock（flock＋pgrep 外部 testpilot run；兩進程搶鎖單測）併入 suite-refuse
- [ ] 2.3 RED＋GREEN：`WinSwCli` driver（Windows 端 serialwrap.exe JSON 解析→持有清單；純解析單測）＋preflight `windows_daemon` 診斷增強（READY 缺項歸因 windows_daemon_holds_device）
- [ ] 2.4 testbed 事實新增 Windows 端 serialwrap.exe 路徑欄位（config.json schema＋文件）

## 3. Phase 1c — hp-cycle 自動救援（realhw）

- [ ] 3.1 RED＋GREEN：救援鏈純決策函式（注入探測結果→動作序列：probe→release→retry≤2→fail attended）——`tests/test_realhw_drivers.py`
- [ ] 3.2 `p1-hp-cycle` 接上救援鏈（subprocess 薄層）＋evidence 記錄＋FailEnv/windows_daemon_holds_device 標註
- [ ] 3.3 真機驗證：`--only p1-hp-cycle`（含 Windows 端持有情境實測）

## 4. Phase 1d — remote 族（realhw）

- [ ] 4.1 `tools/docker/remote_tunnel_test.sh` 加逐拓樸分派參數（`$1`∈{direct,nat_host,dual_nat,gwports,all}，預設 all）
- [ ] 4.2 RED＋GREEN：rm-topo verdict 映射（exit code＋log 尾段→CaseResult 分桶）——`tests/test_realhw_drivers.py`
- [ ] 4.3 `realhw/cases/remote.py`：rm-topo ×4（shell out 包裝、image 延遲建置、teardown finally 掃殘留）
- [ ] 4.4 rm-live ×3（e2e／orphan／cycle：容器 ssh 對端、穿隧道 cmd submit 真板 marker、WAL source 斷言、daemon pid 不變、state dir 淨空；獨立容器名前綴）
- [ ] 4.5 harness `--tier remote` 接線＋`--list` 更新＋checklist/README 文件（R-18）
...
- [ ] 7.1 changelog fragment（R-09）＋docs 對齊（README/checklist，R-16/R-18）
