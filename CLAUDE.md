...
- pytest 下的隔離行為（#120）：`tests/conftest.py` 會硬覆寫 9 個 `SERIALWRAP_*` 目錄變數並 pop 5 個高優先變數（外層 shell export 無效），並於 suite 結束執行 live guard（state/WAL/config/daemon 四維快照比對；逃生閥 `SERIALWRAP_LIVE_GATE=warn`，warn 下結構性破壞仍 FAIL）。
