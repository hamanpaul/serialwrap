...
- `sw_core/platform_backends.py`：**三個平台 seam 選擇器**（#84 PORT-4）——`select_rpc_backend()` / `select_lock_backend()` / `select_device_backend()` 依 `os.name=="nt"` 自動選 `"posix"` 或 `"win"` 後端，並接受環境變數 `SERIALWRAP_RPC_BACKEND` / `SERIALWRAP_LOCK_BACKEND` / `SERIALWRAP_DEVICE_BACKEND`（`auto`/`posix`/`win`）覆寫；純函式、無副作用，可在 POSIX 機器 import 後以 env 強制選 win 後端（用於測試）。
